...
class ConstrainedVAE(torch.nn.Module):
  ...
  def __init__(self, z_dim, k_dim, img_dim=64, img_channels=3):
    super().__init__()
...
if __name__ == '__main__':
    ...
    save_freq = 10
    noise_prob = 0
    savepath = 'constrained_vae/baseline_vae'
    checkpoint = None #'model-50'
    start = 0
    # attr_list = [4, 15]
    ############################
    ...
    for epoch in range(start, start+epochs):
        ...
        for batch, attr in loader:
            ...
            kl_loss = torch.sum(0.5*(mu.pow(2) + log_var.exp() - log_var - 1)) / len(x) # Divide by batch size
            ...
            # attr_mask = torch.zeros(k_dim, dtype=torch.long, device=device)
            # attr_mask[attr_list] = 1
            # common = common * attr_mask.repeat(bs//2, 1)
