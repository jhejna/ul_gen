...
# def random_crop(imgs, out=64, padding=6):
#     """
#         args:
...
#         returns np.array
#     """
#     imgs = F.pad(imgs, (padding, padding, padding, padding), replicate)
#     n, c, h, w = imgs.shape
#     crop_max = h - out + 1
...
# Add Pytorch version of random crop
def random_crop(imgs, out=64, padding=6):
    """
        args:
        imgs: np.array shape (B,C,H,W)
    ...
    if padding > 0:
        imgs = F.pad(imgs, (padding, padding, padding, padding), 'replicate')
    n, c, h, w = imgs.shape
    w1 = torch.randint(0, w - out + 1, (n,))
    h1 = torch.randint(0, h - out + 1, (n,))
    cropped = torch.empty((n, c, out, out), dtype=imgs.dtype, device=imgs.device)
    for i, (img, w11, h11) in enumerate(zip(imgs, w1, h1)):
        cropped[i][:] = img[:, h11:h11 + out, w11:w11 + out]
    return cropped

def random_crop_horizontile(imgs, out=64, padding=7):
    """
        args:
        imgs: np.array shape (B,C,H,W)
        out: output size (e.g. 84)
    ...
    """
    if padding > 0:
        imgs = F.pad(imgs, (padding, padding, 0, 0), 'replicate')
    n, c, h, w = imgs.shape
    crop_max = w - out + 1
    w1 = torch.randint(0, crop_max, (n,))
    cropped = torch.empty((n, c, out, out), dtype=imgs.dtype, device=imgs.device)
    for i, (img, w11) in enumerate(zip(imgs, w1)):
        cropped[i][:] = img[:, :, w11:w11 + out]
    return cropped
...
# New version of grayscale that assumes uint8 input
# def random_grayscale(images, p=.3):
#     """
#         args:
#         imgs: torch.tensor shape (B,C,H,W)
...
# Pytorch version of random cutout
def random_cutout(imgs, min_cut=4,max_cut=24):
    ...
    for img, wc, hc in zip(imgs, w_cut, h_cut):
        w1 = torch.randint(w - wc + 1, ())
        h1 = torch.randint(h - hc + 1, ())
        img[:, h1:h1+hc, w1:w1+wc] = 0
    return imgs


# def random_cutout_color(imgs, min_cut=4,max_cut=24):
#     """
#         args:
#         imgs: shape (B,C,H,W)
#         out: output size (e.g. 84)
